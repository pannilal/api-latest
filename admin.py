...
API_BASE_URL = "http://api.dwanalytics:8001"  # Update this with your API URL

# Add admin credentials
# IMPORTANT: The admin token must be generated by logging in as a user with flags == 9 in the users table.
ADMIN_TOKEN = "REPLACE_WITH_YOUR_ACTUAL_TOKEN"  # Get this by logging in with admin credentials
    ...
    if image:
        async with httpx.AsyncClient() as client:
            ...
            upload_response = await client.post(
                f"{API_BASE_URL}/events/upload-image",
                files=files,
                headers=headers
            )
